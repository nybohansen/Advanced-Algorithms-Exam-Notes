...
Reason about vertex cover in factor 2.
...
The Voronoi diagram is shown as red on figure \ref{8}, while the triangulation is shown as black. A line in the Voronoi diagram separating two vertices, indicates that there is an edge between the two vertices in the triangulation.
...
The algorithm is randomized incremental.


Figure \ref{fig9} shows how the datastructure is maintained when splitting existing triangles and flipping edges.
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig9.pdf}
\caption{Example of maintaining a datastructure when flipping edges and dividing triangles.}
\label{label}
\end{figure}

(i) Convex hull not a triangle, in this case we add an atificial triangle (BIG) wthich sourounds everything. The triangle should be so big that no circle of the points is outside the triangle face <- Pretend this step. Result is that the convex hull of the points are still intact.

How to localize a new point: Datastructure consist of triangles (there three points) and doubly conected list of adjacent triangles (At most three). One node for each triangle


The expected number of edge flips is only linear, worst case is quadratic.
...
Let $P = \{p_1,p_2,\ldots,p_n\}$ be $n$ points in the plane. we 
The definition of 
Given a set of points, \emph{take the convex hull}, add diagonals (edges that connects vertices), without crossing thoose we already have drawn, until there are no more vertices. The result is a triangulation. I.e. we partition the points into triangles.

Not efficient method, edge flip.
Check that a local triangulation is a delaunay triangulation (Draw circle). Edge flip when the circle is not empty. 

Randomized algorithm.
Proposed in the book, incremental edge-flipping algorithm, adding one point at a time.

Edge flips always results in edges connected to the newly added point.



Computational Geometry, Delaunay triangulations (and Voronoi diagrams): From the  book Computational Geometry, Algorithms and Applications, Chapter 9,  Sections 9.1, 9.2, 9.3, and Theorem 9.12 from Section 9.4, available at http://www.cs.uu.nl/geobook,  without the proofs.
