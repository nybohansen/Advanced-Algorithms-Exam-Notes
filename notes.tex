...
By using branch and bound we are trying not to visit the whole solution space, but instead only visit the solution space where we know an optimal solution could reside.

Strong points : 
NP problems
Greedy/Dynamic algorithms
overview on what we try to accomplish
\todo{write some motivation on branch and bound}
...
\label{sub:bounding}
The bounding part of the Branch and Bound algorithm gives a lower (or upper depending on the objective) bound of the solution based on subproblems.

demonstrating how it works on a simple graph, including exp. on live nodes. Presenting one of the methods, 1-tree or LP 
(here is another source for 1-tree \url{http://www-e.uni-magdeburg.de/mertens/TSP/node4.html\#sec:onetree} )
Arguing and maybe proving why is that lower-bound .
% subsection bounding (end)


\subsection{Heuristics} % (fold)
\label{sub:heuristics}
Discussing on the types of representations (permutation etc. ) 
Showing the local maximum issue on hill-climbing 
presenting one advanced methods and mentioning the others.
